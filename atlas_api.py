...
class atlas_request:
    ...
    def uri(self, endpoint):
        ...
        uri_string = (
            self.ATLAS_API_BASE_URL + endpoint + "?api_key=" + self.ATLAS_API_KEY
        )
    ...
    def run_raw(self, endpoint, skip_cache=False):
        ...
        if r.status_code >= 400 and r.status_code < 500:
            raise atlas_request_error(
                "Your request has an error (code %d): %s"
                % (r.status_code, r.json()["status_message"])
            )
        elif r.status_code >= 500:
            if r.text.startswith("{"):
                raise atlas_server_error(
                    "Atlas has errored (code %d): %s"
                    % (r.status_code, r.json()["status_message"])
                )
            else:
                raise atlas_server_error(
                    "Atlas has errored (code %d): %s" % (r.status_code, r.text)
                )
        ...
        if raw_json["status"] != "OK":
            raise atlas_server_error(
                "Atlas a bad status with no error code. Status: %s, Status message: %s",
                raw_json["status"],
                raw_json.get("status_message", ""),
            )
    ...
    def influence_distribution(self):
        return self.run("influence-distribution").output
